...
#
set feeds(pulina:#pulina) {10 http://pulina.info/feed}
set feeds(kommentit:#pulina) {10 http://pulina.info/comments/feed}
#
...
proc process {data chanfeed} {
	...
	foreach {foo item} [regexp -all -inline {(?i)<item.*?>(.*?)</item>} $data] {
		...
		if {[lsearch -exact $hash($chanfeed) [md5 $title]] == -1 && [botonchan $chan]} {
			if {![info exists header]} {
				...
				puthelp "privmsg $chan :\002PÄIVITYS\002 - $header"
			}
			if {$count < $maxnew} {
	...
	if {[info exists indices] && [botonchan $chan]} {
		...
		puthelp "privmsg $chan :...ja vielä $count muuta $indices" 
	}
...
proc rss {hand idx text} {
	variable feeds
	if {$text == ""} {
		putdcc $idx "Käyttö: .$::lastbind <add|del|list> \[name:#chan \[feed\]\]"
		return
	}
	set text [split $text]
	switch [lindex $text 0] {
		...
		"add" {
			if {[llength $text] < 4} {
				putdcc $idx "ei tarpeeksi argumentteja"
				return
			}
			...
			if {[lsearch -exact $names [string tolower $chanfeed]] != -1} {
				putdcc $idx "$chanfeed on jo olemassa"
				return
			}}
		...
		"del" {
			...
			set names [string tolower [array names feeds]]
			if {[lsearch -exact $names [string tolower $chanfeed]] == -1} {
				putdcc $idx "$chanfeed ei ole olemassa"
				return
			} {
...
proc news {nick uhost hand chan text} {
	...
	if {$text == ""} {
		...
		if {[info exists names]} {
			...
			puthelp "notice $nick :feedit kanavalle $chan: $names"
			puthelp "notice $nick :tyyppi $::lastbind <feed> \[index#\]"
		} {
			puthelp "notice $nick :ei feedejä $chan"
		}
		return 1
	}
	if {![info exists news($feed:$chan)]} {
		puthelp "notice $nick :Ei uutta sivustolla $feed kanavalla $chan"
		return 1
	}
	if {$num == ""} {
		set idx 1
		...
		puthelp "notice $nick :uutisen täytyy olla numero"
		return 1
	}
	if {$num < 1 || $num > [llength $news($feed:$chan)]} {
		puthelp "notice $nick :ei tuon numeroista uutista, kokeile $::lastbind $feed"
		return 1
	} {
		set idx [expr {$num-1}]
...
putlog "Script loaded: \002Rss News\002"