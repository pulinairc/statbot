...
# (list)
set twitter(accts) {
	"#pulina|pulinainfo|MqqjJ40CMIa35fv7vEEDg|Otsdb6Oa4i9EW0fcedLyRvvxtPSA2LB2bO2MWESPZw|178017566-LQqKIw2Zsrp8O2jhSgia1lRBe34Lo1rYmo1VT5H8|43Y5joSCQt88HAQTmt8y3aITJUV59HCFgMonsSVOuLw"
#	"#rolletestaa123|pulinainfo|MqqjJ40CMIa35fv7vEEDg|Otsdb6Oa4i9EW0fcedLyRvvxtPSA2LB2bO2MWESPZw|178017566-LQqKIw2Zsrp8O2jhSgia1lRBe34Lo1rYmo1VT5H8|43Y5joSCQt88HAQTmt8y3aITJUV59HCFgMonsSVOuLw"
}
...
# (strings)
#set twitter(tweet-display) "Tweetti luotu: http://twitter.com/%user%%trunc% ( %id%@%user% - %created% via %source% )"
...
bind pub o !tweet proc:tweet
#bind pubm - {*://*} proc:tweet
bind pubm - {*kalja*} proc:tweet
#bind pubm - {*tissit*} proc:tweet
bind pubm - {*olut*} proc:tweet
bind pubm - {*bisse*} proc:tweet
bind pubm - {*sammunu*} proc:tweet
bind pubm - {*känni*} proc:tweet
...
proc proc:tweet {nick uhand handle chan input} {
	...
	# a flagged bot known tweeter?
	# if {![matchattr $handle $twitter(flags) $chan] && ![info exists allow]} {
	# 	putserv "$to :$twitter(logo) Sinulla ei ole oikeutta tweetata kanavalla. Pittää olla ($twitter(flags) flags), tai ([join $twitter(channel)]) kanavalla $chan. Sulla ei ole kumpaakaan. :P"
	# 	return
	# }
	if {[info exists twitter(noutf8)]} {
		set http [::http::config -useragent $twitter(agent)]
...
if {$nick == "rolle"} { 
set tweetform "$ret$ment[string map [list %nick "<@rolle>" %chan $chan %text "$input" "%modes" "[proc:tweet:modes $nick $chan]"] $twitter(globaltweetformat)]"
} elseif {$nick == "helyes"} { 
set tweetform "$ret$ment[string map [list %nick "<@HelyesH>" %chan $chan %text "$input" "%modes" "[proc:tweet:modes $nick $chan]"] $twitter(globaltweetformat)]"
} elseif {$nick == "samiy"} {
set tweetform "$ret$ment[string map [list %nick "<@samiylitalo>" %chan $chan %text "$input" "%modes" "[proc:tweet:modes $nick $chan]"] $twitter(globaltweetformat)]"
} elseif {$nick == "hekez"} {
set tweetform "$ret$ment[string map [list %nick "<@Hekez>" %chan $chan %text "$input" "%modes" "[proc:tweet:modes $nick $chan]"] $twitter(globaltweetformat)]"
} elseif {$nick == "firewalker"} {
set tweetform "$ret$ment[string map [list %nick "<@Megadeathfire>" %chan $chan %text "$input" "%modes" "[proc:tweet:modes $nick $chan]"] $twitter(globaltweetformat)]"
} elseif {$nick == "mustikkasoppa"} {
set tweetform "$ret$ment[string map [list %nick "<@mustikkasoppa>" %chan $chan %text "$input" "%modes" "[proc:tweet:modes $nick $chan]"] $twitter(globaltweetformat)]"
} elseif {$nick == "j-tek"} {
set tweetform "$ret$ment[string map [list %nick "<@jtekproductions>" %chan $chan %text "$input" "%modes" "[proc:tweet:modes $nick $chan]"] $twitter(globaltweetformat)]"
} elseif {$nick == "Yoa"} {
set tweetform "$ret$ment[string map [list %nick "<@Yoaboa>" %chan $chan %text "$input" "%modes" "[proc:tweet:modes $nick $chan]"] $twitter(globaltweetformat)]"
} elseif {$nick == "incidence"} {
set tweetform "$ret$ment[string map [list %nick "<@incdidence>" %chan $chan %text "$input" "%modes" "[proc:tweet:modes $nick $chan]"] $twitter(globaltweetformat)]"
} elseif {$nick == "Vanitsu"} {
set tweetform "$ret$ment[string map [list %nick "<@Vanitsu>" %chan $chan %text "$input" "%modes" "[proc:tweet:modes $nick $chan]"] $twitter(globaltweetformat)]"
} elseif {$nick == "sirhoksu_"} {
set tweetform "$ret$ment[string map [list %nick "<@Hoksu>" %chan $chan %text "$input" "%modes" "[proc:tweet:modes $nick $chan]"] $twitter(globaltweetformat)]"
} elseif {$nick == "sirhoksu"} {
set tweetform "$ret$ment[string map [list %nick "<@Hoksu>" %chan $chan %text "$input" "%modes" "[proc:tweet:modes $nick $chan]"] $twitter(globaltweetformat)]"
} elseif {$nick == "Naama"} {
set tweetform "$ret$ment[string map [list %nick "<@naamattu>" %chan $chan %text "$input" "%modes" "[proc:tweet:modes $nick $chan]"] $twitter(globaltweetformat)]"
} elseif {$nick == "alajarvela"} {
set tweetform "$ret$ment[string map [list %nick "<@alajarvela>" %chan $chan %text "$input" "%modes" "[proc:tweet:modes $nick $chan]"] $twitter(globaltweetformat)]"
} else {
set tweetform "$ret$ment[string map [list %nick "<$nick>" %chan $chan %text "$input" "%modes" "[proc:tweet:modes $nick $chan]"] $twitter(globaltweetformat)]"
	...
	if {[info exists replyid]} {
		...
		if {[catch {set html [proc:twitter:oauth $twitter(tweet) POST $chan $q]} error]} { 
			#putserv "$to :$twitter(logo) $error" ; 
			return 
		}
	...
	if {[string match "*\"error*" $html]} {
		...
		if {![info exists dm]} {
			...
			set tt [string map [list %created% "[proc:twitter:duration [string trim $chek]]" %source% "[string trim $s]" %id% "$id" %user% "$tuser" %chan% "$chan" %trunc% "$cut"]]
