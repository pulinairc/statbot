...
proc webby {nick uhost handle chan site} {
  ...
  if {[string equal "-" [string index [set site [string map {"  " " "} [string trim $site]]] 0]]} {
   putserv "privmsg $chan :\[ \002Virhettä pukkaa\002: you've used an improper flag which may exploit http-package! \]"
   return
  }
  ...
  if {[info exists w4]} {
    ...
    if {![regexp -- {\|} $fullquery] && ![regexp -- {\?} $fullquery]} {
     putserv "privmsg $chan :\[ \002Virhettä pukkaa\002: $fullquery does not appear to be a post query, using simple query... \]"
      unset w4
    } else {
      if {[regexp -- {\|} $fullquery]} {
       ...
       putserv "privmsg $chan :\[ \002Virhettä pukkaa\002: detected \002url\002 $url \002query\002 $query\]"
      } else {
        set suburl ""
        set url [lindex [split $fullquery "?"] 0]
        set query [lindex [split $fullquery "?"] 1]
      ...
      foreach {entry} [split $query "&"] {
        if {![regexp {\=} $entry]} {
         putserv "privmsg $chan :\[ \002Virhettä pukkaa\002: broken post structure ($entry). Is not post query, using simple query...\]"
          unset w4
          break
        } else {
  ...
  if {![string match -nocase "::http::*" $error]} {
    ...
    putserv "privmsg $chan :\[ \002Virhettä pukkaa\002: $cleanerr \( $fullquery \)\]"
     return 0
  }
  if {![string equal -nocase [::http::status $http] "ok"]} {
    putserv "privmsg $chan :\[ \002Virhettä pukkaa\002: [string totitle [::http::status $http]] \( $fullquery \)\]"
    return 0
  }
  upvar #0 $http state
  if {![info exists state(meta)]} { putserv "privmsg $chan :\[ \002Virhettä pukkaa\002: unsupported URL error \( $fullquery \)\]" ; return 0 }
  ...
  if {[string match *t.co* $fullquery] && [regexp {<META http-equiv="refresh" content="[0-9]\;URL\=(.*?)">} $html - value]} {
    ...
    if {![string match -nocase "::http::*" $error]} {
     putserv "privmsg $chan :\[ \002Virhettä pukkaa\002: [string totitle $error] \( $value \)\]"
      return 0
    }
    if {![string equal -nocase [::http::status $http] "ok"]} {
     putserv "privmsg $chan :\[ \002Virhettä pukkaa\002: [string totitle [::http::status $http]] \( $value \)\]"
      return 0
    }
    upvar #0 $http state
   if {![info exists state(meta)]} { putserv "privmsg $chan :\[ \002Virhettä pukkaa\002: unsupported URL error \( $fullquery \)\]" ; return 0 }
      ...
      if {[regexp -nocase ^location$ $name]} {
        ...
        if {[string match [string map {" " "%20"} $value] $url]} {
          if {![info exists poison]} {
            ...
            incr poison
            if {$poison > 2} {
             putserv "privmsg $chan :\[ \002Virhettä pukkaa\002: redirect error (self to self)\($url\) - ($cookies) are no help...\]"
              return
            }
          }
        }
        ...
        set url [string map {" " "%20"} $value]
        upvar #0 $http state
       if {[incr r] > 10} { putserv "privmsg $chan :\[ \002Virhettä pukkaa\002: redirect error (>10 too deep) \( $url \)\]" ; return }
        # iterate through the meta array
        #if {![string length cookies]} {
          foreach {name value} $state(meta) {
            # do we have cookies?                                                                                                                                                                             
    ...
    if {![string equal -nocase $char2 $char3] && ![string equal -nocase "none given" $char2] && $::webbyFixDetection > 0} {
      if {$::webbyFixDetection > 0} {
        switch $::webbyFixDetection {
          1 { if {![info exists w8]} {
               if {$::webbyShowMisdetection > 0 } { set flaw "\[ \002Virhettä pukkaa\002: conflict! html meta tagging reports: $char2 .. using charset detected from http-package: $char3 to avoid conflict.\]"} { set flaw "" }
        set html [webbyConflict $html $char3 $char2 2 [info exists w9]]
                set char [string trim $char3 {;}]
              } else {
               if {$::webbyShowMisdetection > 0 } { set flaw "\[ \002Virhettä pukkaa\002: conflict! http-package reports: $char3 .. using charset detected from html meta tagging: $char2 to avoid conflict.\]" } { set flaw "" }
  ...
  if {$::webbyShowMisdetection > 0} {
    ...
    if {$::webbyShowMisdetection > 1} {
     putserv "privmsg $chan :\[ \002Virhettä pukkaa\002: Detected \002$char\002 \:\: Http-Package: $cset -> $char3 .. Meta-Charset: $mset -> $char2 \:\:"
    }
  }
