...
set isUsersOwn "En ole nähnyt sinua vielä. Sano jottai."
#
## 2.When users supply an other nick and bot has NOT seen it yet:
set isOtherUser "En ole nähnyt tuota nimimerkkiä vielä. Ehkä hän ei ole sanonut vielä mitään."
#
## 3.When users supply the bots own name:
set isBotsName "Näin mut just nyt! Just tässä! Ylläri?"
#
# ----------------------------------------------------------
# Seen Results
# (0=disable,1=enable)
...
namespace eval seend {
   ...
   proc dcc {handle idx text} {
      ...
      if {$text == "" || $text == "help"} {
         # show help.
         variable ::seend::ver
         putdcc $idx "Käyttö: .[set ::seend::dccCmd] ?backup|restore?"
      } elseif {$text == "backup"} {
   ...
   proc pub_msg_save {nick userhost handle channel text} {
      ...
      if {[channel get $channel seend]} {
         ...
         if {$first != [set ::seend::pubCmd]} {
            ...
            if {$logseens} {
               putlog "Uusi .seen -kutsu nimimerkiltä $nick: $text"
            }
         }
      }
   }
   proc get_info {nick who} {
      ...
      if {[set ::seend::showduration] == 1} {
         set last "[duration [expr {[clock seconds] - $time}]] sitten"
      } else {
         set last [string map {"\n" ""} [clock format $time -format {%Y/%m/%d %H:%M:%S}]]
      }
      if {[set seend::showchannel]} {
         # showchannel is enabled. Add last channel.
         set chanmsg " kanavalla $lastchan($who)"
      }
      if {[set seend::showmessage]} {
         # show message is enabled. Add last message.
         set storedmsg " Viimeisin viesti: $lastmsg($who)"
      }
      if {$nick == $who} {
         set result "Näin sinut viimeksi $last${chanmsg}.${storedmsg}"
      } else {
         set result "Näin viimeksi nimimerkin $who $last${chanmsg}.${storedmsg}"
      }
      return $result
   }
   proc pub_show_seen {nick userhost handle channel text} {
      ...
      if {[channel get $channel seend]} {
         ...
         if {$ctext == "help"} {
            ...
            variable ::seend::ver
            putserv "PRIVMSG $channel :Käyttö: [set ::seend::pubCmd] ?nick|help|\*?"
            ...
            if { $namelist != "" } {
               ...
               putserv "PRIVMSG $channel :${name}Hakusarja '${otext}' mätsää: ${namelist}"
            } else {
               # no names match pattern.
               putserv "PRIVMSG $channel :${name}Ei mätsiä hakusarjalle '${otext}'."
            }
